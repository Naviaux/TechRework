Hooks:PostHook(PlayerManager, "on_killshot", "PercusiveMaintenanceBasic", function (self, killed_unit, variant)
	...
	if not (variant == "melee") or (managers.skilltree:skill_step("t_left6") == 0) then
		log ("Failed Check")
		return
	end
	log ("PASS Check")
	if managers.player:upgrade_value("player", "melee_ammo_replenish_chance") >= math.rand(1) then
		...
		for i = 1, 2 do
			...
			weapon:base()._ammo_total = weapon:base():digest_value(current_ammo + max_ammo * managers.player:upgrade_value("player", "melee_ammo_replenish"), true)
		end
	end
end)